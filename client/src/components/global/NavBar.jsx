...
import {
    ...
    USER_ROUTE,
    ACCOUNT_ROUTE,
    BASE_ROUTE, MENU_ROUTE, CONFIG_ROUTE, SEARCH_ROUTE, HOME_ROUTE, STATIC_FILE_URL
} from '../../utils/consts'
...
import ModalLeftMenuMobile from "../menu/ModalLeftMenuMobile.jsx";
        ...
        <div className='d-flex W-100 flex-wrap '>
          <div className='d-flex flex-wrap ms-3 m-2 '>
              ...
              <div className='NavBarLeftDiv flex-row'>
                  ...
                  {
                      user.isAuth && user.info ?
                          <span className='fs-4 me-2 text-light'>{ user.info.projectName == null ? 'База знаний' : 'База знаний - ' + user.info.projectName}</span>
                          :
                          <span className='fs-4 me-2 text-light'>{'База знаний'}</span>
                  }

                  {
                      user.isAuth ?
                      <div>
                          <Button variant='outline-light ms-2 me-2 fs-5 pt-1 pb-1' onClick={()=>navigate(MENU_ROUTE)}>Объекты</Button>
          ...
          </div>
            {
                ...
                <div className='MobileHide'>
                    <div className='mt-auto mb-auto d-flex flex-wrap h-100'>
                        ...
                        <Button className='ms-2 mt-auto mb-auto p-1' variant='outline-light'
                                onClick={() => SearchFun(searchText)}>
                            <Image className='m-0 p-0 Black' src={iconSearchW}/>
                            <Image className='m-0 p-0 White' src={iconSearchB}/>
                        </Button>
                    </div>
                </div>: null
            }
          {user.info && user.isAuth ?
            <Nav className="ms-auto me-3">
              <span className='m-2 text-white d-flex align-items-center'>{user.info.name}</span>
                ...
                <NavDropdown align={'end'} title={<Image width={36} height={36} src={STATIC_FILE_URL+
