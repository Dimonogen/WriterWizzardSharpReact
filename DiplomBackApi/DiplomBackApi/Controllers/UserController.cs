...
public class UserController : MyBaseController
{
    ...
    public async Task<ActionResult> Login([FromBody] LoginModel loginModel)
    {
        ...
        if ( user == null )
        {
            _logger.LogError("User not found. Error login. Пользователь не найден.");
            return BadRequest(new ResponseDtoError( "Ошибка, неверная комбинация логина и пароля.") );
        }
    ...
    public async Task<ActionResult> Registration([FromBody] RegistrationModel model)
    {
        ...
        if (users.Count > 0)
        {
            return BadRequest(new ResponseDtoError("Пользователь с таким email уже существует. Регистрация невозможна.") );
        }
